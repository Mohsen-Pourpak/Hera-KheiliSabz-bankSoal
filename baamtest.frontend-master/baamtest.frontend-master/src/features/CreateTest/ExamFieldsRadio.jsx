...
export function ExamFieldsRadio({
  ...
  const submitChanges = (event) => {
    const filtered_grades = gradesLabel.filter(item => {return item !== undefined})
    const filtered_fields = fieldsLabel.filter(item => {return item !== undefined})
      ...
      items.map(item => {
        return (
          <MenuItem value={item.id}>
            <img src={check.includes(item.id) ? CheckRadioIcon : UncheckRadioIcon} style={{height: 20, margin: '5px 10px'}} />
            {item.title}
          </MenuItem>
        );
      })
  ...
  return (
    ...
    >
      ...
      <Select
        id="gradeSelect"
        open={gradeFilter}
        onOpen={() => {setGradeFilter(true)}}
        onClose={() => {setGradeFilter(false)}}
        value={gradesLabel}
        style={{ width: "100%",  borderRadius: 10, padding: "10px" }}
        input={<NormalInput  style={{ padding: "0.5em" }} placeholder="پایه" />}
        onChange={e => valueChange("gradeId", e)}
        renderValue={(selected) => {
          return (
            <p style={{ color: "#a6a6a6", fontSize: "0.9rem", fontFamily: "Dana" }}>{handleGrades(selected)}</p>
          )
        }}
        displayEmpty
        multiple
      ...
      >
        <MenuItem>
          پایه
          <CloseIcon style={{ color: 'red', marginRight: '90px' }} onClick={() => {setGradeFilter(false)}} />
        </MenuItem>
      ...
      <Select
        ...
        style={{ width: "100%",  borderRadius: 10, padding:"10px" }}
        open={fieldFilter}
        onOpen={() => {setFieldFilter(true)}}
        onClose={() => {setFieldFilter(false)}}
        input={<NormalInput style={{ padding: "0.5em" }} />}
        onChange={e => valueChange("fieldId", e)}
        renderValue={(selected) => {
          return (
            <p style={{ color: "#a6a6a6", fontSize: "0.9rem", fontFamily: "Dana" }}>{handleFields(selected)}</p>
          )
        }}
        displayEmpty
        multiple
      ...
      >
        <MenuItem >
          رشته
          <CloseIcon style={{ color: 'red', marginRight: '90px' }} onClick={() => {setFieldFilter(false)}} />
        </MenuItem>
      ...
      <Button
        ...
        color="primary"
        size="small"
        style={{fontSize: "0.9rem", textAlign : 'center', fontFamily: "Dana", height: 45, borderRadius: 10}}
        >
        اعمال
      </Button>
    </div>
