...
import { subStudents, uploadUsersExcel } from "../../api/services/user";
import { freeAdvisorsSc, assignStudentSc } from "../../api/services/school";
...
import MySample from "./excel-samples/StuSample.xlsx";
...
class CreateTest extends React.Component {
  ...
  assignOpacity = () => {
    let token = localStorage.getItem("userToken");
    let data = [this.state.userSearched.id];
    assignStudentSc(data, token).then(res => {
      console.error(res);
    ...
    return this.state.subAdvisors.map(item => {
      return (
        ...
        >
          ...
          <div style={{ ...styles.trItem, flex: 1 }}>{item.fullName}</div>
          <div style={{ ...styles.trItem, flex: 1 }}>{item.fieldTitle}</div>
  ...
  onUploadClick = () => {
    ...
    let token = localStorage.getItem("userToken");
    uploadUsersExcel(token, "1", this.state.file, `${this.state.rowsCount}`)
      .then(res => {
        console.error({ res });
        ...
        {this.state.isLoading ? (
          ...
          <Grid container item xs={12} style={{ padding: "0 10px" }}>
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ ...styles.thItem, flex: 1 }}>رشته</div>
            ...
            >
              ...
              <a href={MySample} download="StudentSample.xlsx">
                <Button
                  variant="contained"
                  color="primary"
