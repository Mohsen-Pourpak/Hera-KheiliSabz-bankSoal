...
import {Delete as DeleteIcon, Edit as EditIcon, Lock, AccountBalanceWallet, FilterList, Cancel, AddCircle} from '@material-ui/icons'
...
import { addStudent, deleteBook, deleteStudent, editBook, editStudent, getBook, getStudent, getAllStudents, changeUserPassword, chargeUserBalance } from "../../api/services/admin";
...
const Students = (props) => {
  ...
  const onEditClick = async item => {
    ...
    let res = await getStudent(item.id, token)
  ...
  const onDeleteClick = item => {
    ...
    deleteStudent(item.id, token).then(res => {
      if (res.isSuccess) {
  ...
  const fetchData = async () => {
    setOpenFilter(false)
    let token = localStorage.getItem('userToken')
    let studentsData = await getAllStudents(filters, token, (page+1), perPageTable)
  ...
  const onSubmit = () => {
    ...
    if (dialogType === 'chargeBalance') {
      ...
      addStudent(state, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false)
          setState(defaultData)
          toast.success(res.message)
      ...
      editStudent(removeKeys(state, ['id']), userId, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false)
          setState(defaultData)
          toast.success(res.message)
  ...
  return (
    <div style={{marginTop: 10}}>
      ...
      <PageTitle style={{width: 'fit-content'}} title="دانش آموزان" />
      <div style={{flex: 1}} />
      <Button
          variant="outlined"
          onClick={() => {
    ...
    <AddDialog
      ...
      title={
       ...
       'تغییر رمزعبور' : `${dialogType === 'edit' ? 'ویرایش' : 'افزودن'} دانش آموز`
      }
