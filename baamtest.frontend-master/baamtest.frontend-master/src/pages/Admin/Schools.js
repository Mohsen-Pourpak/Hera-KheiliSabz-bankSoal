...
const Schools = props => {
  ...
  const onEditClick = async item => {
    ...
    let res = await getSchool(item.id, token);
  ...
  const onDeleteClick = item => {
    ...
    deleteSchool(item.id, token).then(res => {
      if (res.isSuccess) {
        toast.success(res.message);
        setStudents(sliders => {
          return [
            ...sliders.slice(0, foundIndex),
            ...sliders.slice(foundIndex + 1),
          ];
        });
      }
    });
  ...
  const transform = data => {
    return data.map((item, idx) => {
      ...
      return [
        ...
        item.name,
        getSchoolTypeDisplay(item.schoolType),
        item.managerName,
        toFA(item.postalCode),
        toFA(item.schoolPhone),
  ...
  const fetchData = async () => {
    ...
    let pagination = JSON.parse(schoolsData.headers.pagination);
    setCount(pagination.totalItems);
    setStudents(schoolsData.data.data);
  };
  ...
  const onSubmit = () => {
    ...
    if (dialogType === "chargeBalance") {
      ...
      addSchool(state, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false);
          toast.success(res.message);
          fetchData();
      ...
      editSchool(removeKeys(state, ["id"]), userId, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false);
          toast.success(res.message);
          fetchData();
  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      >
        <PageTitle style={{ width: "fit-content" }} title="مدارس" />
        <div style={{ flex: 1 }} />
        <Button
          variant="outlined"
          onClick={() => {
        ...
        title={
          dialogType === "chargeBalance"
            ...
            : dialogType === "changePassword"
              ? "تغییر رمزعبور"
              : `${dialogType === "edit" ? "ویرایش" : "افزودن"} مدرسه`
        }
