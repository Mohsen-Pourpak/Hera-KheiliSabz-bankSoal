...
const Advisors = (props) => {
  ...
  const onEditClick = async item => {
    ...
    let res = await getAdvisor(item.id, token)
  ...
  const onDeleteClick = item => {
    ...
    deleteAdvisor(item.id, token).then(res => {
      if (res.isSuccess) {
  ...
  const fetchData = async () => {
    setOpenFilter(false)
    let token = localStorage.getItem('userToken')
    let studentsData = await getAllAdvisors(filters, token, (page+1), perPageTable)
  ...
  const onSubmit = () => {
    ...
    if (dialogType === 'chargeBalance') {
      ...
      addAdvisor(state, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false)
          toast.success(res.message)
          fetchData()
      ...
      editAdvisor(removeKeys(state, ['id']), userId, token).then(res => {
        if (res.isSuccess) {
          setOpenDialog(false)
          toast.success(res.message)
          fetchData()
  ...
  return (
    <div style={{marginTop: 10}}>
      ...
      <PageTitle style={{width: 'fit-content'}} title="مشاوران" />
      <div style={{flex: 1}} />
      <Button
          variant="outlined"
          onClick={() => {
    ...
    <AddDialog
      ...
      title={
        ...
        'تغییر رمزعبور' : `${dialogType === 'edit' ? 'ویرایش' : 'افزودن'} مشاور`
      }
