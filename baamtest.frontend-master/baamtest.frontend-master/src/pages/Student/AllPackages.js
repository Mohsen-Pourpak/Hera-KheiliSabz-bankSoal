...
class AllPackages extends React.Component {
  ...
  componentDidMount() {
    ...
    getAllQuestionPackv2(token)
      .then(res => {
        this.setState({
          packages: res.data,
          newPackages: res.data,
          progress: false,
        });
      })
      .then(() => {
  ...
  onFilter = (id1, id2) => {
    ...
    this.setState({ newPackages }, () =>
      this.handlePagination(newPackages.length),
    );
  };
                ...
                >
                  ...
                  >
                    {this.state.grades.map(el => {
                      return (
                        <div
                          onClick={() => this.onFilter(el.id, null)}
                          style={{
                            flexDirection: "row",
                            cursor: "pointer",
                            display: "flex",
                        ...
                        >
                          <img
                            src={
                              this.state.selectedGrades.includes(el.id)
                                ? CheckRadioIcon
                                : UncheckRadioIcon
                            }
                ...
                >
                  ...
                  >
                    {this.state.fields.map(el => {
                      return (
                        <div
                          onClick={() => this.onFilter(null, el.id)}
                          style={{
                            flexDirection: "row",
                            cursor: "pointer",
                            display: "flex",
                        ...
                        >
                          <img
                            src={
                              this.state.selectedFields.includes(el.id)
                                ? CheckRadioIcon
                                : UncheckRadioIcon
                            }
                            style={{ height: 20, margin: "5px 10px" }}
                      ...
                      this.state.pageData[this.state.page - 1].map(el => {
                        return (
                          <div style={{ width: "20%", padding: 20 }}>
                            <img
                              src={
                                el.image ? `${BASE_URL}${el.image}` : bookImage
                              }
                              width="100%"
                            />
                            <div
                            ...
                            >
                              قیمت : {toFA(el.price.toLocaleString())} تومان
                            </div>
                            <Button
                              onClick={() =>
                                this.props.history.push({
                                  pathname: `/dashboard/packages-buy/${el.bookId}`,
                                })
                              }
                              fullWidth
                              variant="contained"
