...
import { dateTimeToJalaliWithTime, getDateTime, PER_PAGE_TABLES, toFA, WALLET_TYPES } from '../../utils/Utils';
        ...
        return this.state.payments.length === 0 ? (
            ...
            return (
                ...
                >
                    ...
                    <div style={{...styles.trItem, width: '50%'}}>{item.description}</div>
                    <div style={{...styles.trItem, width: 150}}>{toFA(item.amount.toLocaleString())} تومان</div>
                    <div style={{...styles.trItem, width: 100}}>{WALLET_TYPES.filter(el => el.value === item.walletType)[0].title}</div>
                    <div style={{...styles.trItem, width: 150}}>{getDateTime(item.createDate)}</div>
                    <div style={{...styles.trItem, width: 100, backgroundColor: item.isPay ? '#deffd5' : '#ffebe4', color: item.isPay ? '#329455' : '#ec6058'}}>{item.isPay ? <CheckCircle style={{fill: '#329455', fontSize: '90%', marginLeft: 5}} /> : <Cancel style={{fill: '#ec6058', fontSize: '90%', marginLeft: 5}} />}{item.isPay ? 'موفق' : 'ناموفق'}</div>
                </div>
            )
        })
    }
        ...
        return (
            <>  
                ...
                {this.state.isLoading ? (
                    <Backdrop style={{zIndex: 1000000, color: '#3d82a4'}} open={this.state.isLoading} onClick={() => console.log('clicked')}>
                                ...
                                <Grid item sm={12} spacing={1} alignItems="center" className='inputContainer' style={{padding: '7.5px', margin: 0, width: '100%'}}>
                                    <div style={{flexDirection: 'column', display: 'flex', alignItems: 'center', width: '100%'}}>
                                        <div style={{display: 'flex', width: '100%', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 10}}>
                                            ...
                                            <div style={{...styles.thItem, width: '50%'}}>توضیحات</div>
                                            <div style={{...styles.thItem, width: 150}}>مبلغ</div>
                                            <div style={{...styles.thItem, width: 100}}>نوع تراکنش</div>
                                            <div style={{...styles.thItem, width: 150}}>تاریخ و ساعت</div>
